...
@router.get('/{wallet_uuid}', response_model=SWalletDB, status_code=status.HTTP_200_OK)
...
@router.post('/{wallet_uuid}/operation', response_model=SWalletDB, status_code=status.HTTP_201_CREATED)
async def wallet_operation(wallet_uuid: str, operation_type: SWalletUpdate,
...
@router.post('/', response_model=SWalletDB, status_code=status.HTTP_201_CREATED)
async def create_new_wallet(wallet: SWalletCreate, session: AsyncSession = Depends(get_async_session),):
    """
    Создать кошелек
    """
